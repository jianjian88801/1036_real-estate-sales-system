<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="房子户型" prop="fangzihuxing">
          <el-input v-model="ruleForm.fangzihuxing" 
              placeholder="房子户型" clearable  :readonly="ro.fangzihuxing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="房子户型" prop="fangzihuxing">
              <el-input v-model="ruleForm.fangzihuxing" 
                placeholder="房子户型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                              </el-row>
  ...
  data() {
	...
	fangzihuxing : false,
            ...
            ruleForm: {
                          ...
                          rules: {
                  fangzihuxing: [
                            { required: true, message: '房子户型不能为空', trigger: 'blur' },
                                    	                                                              ],
              }
    };
  },
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          	            if(o=='fangzihuxing'){
            this.ruleForm.fangzihuxing = obj[o];
	    this.ro.fangzihuxing = true;
            continue;
          }
	                    }
                              }
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `fangzihuxing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fangzihuxingCrossAddOrUpdateFlag = false;
                  this.parent.search();
